...
interface IServiceAidlCall {
    ...
    //注意:
    //uiPost接口在客户端远程调用后将会运行在服务端当前的binder线程中，服务端运行完成后将会回调至客户端的调用线程。
    //  客户端在哪个线程远程调用uiPost，就会在哪个线程回调。
    ...
    //注意：
    //asynPost接口在客户端远程调用后服务端将会开启新的线程执行，所有开启的线程是并行的，执行完成后将会回调在客户端的binder工作线程。
